...
async def extract_page_listings(page: Page, config: dict) -> list:
    ...
    try:
        raw_elements = await page.evaluate("""(cfg) => {
            ...
            return Array.from(cards).map(card => {
                const titleNode = card.querySelector(cfg.selectors.role_title);
    ...
    blacklist_keywords= {"unpaid", "free", "stipendless", "volunteer", "nil", "none", 
            "performance based", "incentive", "incentives", "expenses covered",
