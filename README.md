...
A local automation tool that scans [Internshala](https://internshala.com) for relevant internships, stores discoveries in a local vault, and (optionally) fills application forms using your resume context and a local LLM (Ollama).

Runs entirely on your machine — browser session, job database, and resume data stay local.
...
Default API: `http://localhost:11434`, model `llama3.2` (see `LLMResponseSynthesizer` in `applier.py`).
